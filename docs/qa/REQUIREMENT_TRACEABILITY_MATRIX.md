...
| 模块 | 需求项 | 实现证据 | 自动化证据 |
|---|---|---|---|
| 双人绑定 | 二维码绑定、建立关系、生成 pair_id | `lib/src/features/bonding/bonding_page.dart`、`scan_bind_page.dart`、`lib/src/data/pair_repository.dart` | `./scripts/qa/mvp_self_check.sh` 文件与关键字检查 |
...
| 同步缺失事件 | `SyncSession.buildSyncRequest/buildSyncResponse/buildDeltaSyncPushPayload` | `test/sync_session_test.dart` |
...
- `scripts/qa/mvp_self_check.sh` 中“本地优先与无服务端守卫”检查：

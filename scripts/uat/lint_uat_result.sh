...
UAT_FILE="${UAT_FILE:-docs/qa/NEARBY_DUAL_DEVICE_UAT_RESULT.md}"
if [[ "$UAT_FILE" != /* ]]; then
...
if [[ "${#errors[@]}" -gt 0 ]]; then
  ...
  echo
  echo "[uat-lint] 处理建议："
  echo "  1) 按 docs/qa/NEARBY_DUAL_DEVICE_UAT.md 完成双机测试"
  echo "  2) 填写 docs/qa/NEARBY_DUAL_DEVICE_UAT_RESULT.md 所有字段与截图路径"
  echo "  3) 先运行 ./scripts/uat/lint_uat_result.sh，清零问题后再运行 ./scripts/uat/check_uat_result.sh"
  exit 1
fi

echo "[uat-lint] 预检通过，可执行 ./scripts/uat/check_uat_result.sh"

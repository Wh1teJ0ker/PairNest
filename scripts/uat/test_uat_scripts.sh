...
run_expect 0 env UAT_FILE="$PASS_FILE" ./scripts/uat/lint_uat_result.sh
run_expect 0 env UAT_FILE="$PASS_FILE" ./scripts/uat/check_uat_result.sh

echo "[uat-test] 场景2：存在未勾选项应被拦截"
run_expect 1 env UAT_FILE="$PENDING_FILE" ./scripts/uat/lint_uat_result.sh
run_expect 2 env UAT_FILE="$PENDING_FILE" ./scripts/uat/check_uat_result.sh

echo "[uat-test] 场景3：占位符未替换应被拦截"
run_expect 1 env UAT_FILE="$PLACEHOLDER_FILE" ./scripts/uat/lint_uat_result.sh
run_expect 3 env UAT_FILE="$PLACEHOLDER_FILE" ./scripts/uat/check_uat_result.sh

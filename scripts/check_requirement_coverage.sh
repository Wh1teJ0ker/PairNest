...
render_row "B-1" "双人绑定" "二维码绑定与加入" "需真机验证" "docs/NEARBY_DUAL_DEVICE_UAT.md"
...
render_row "T-2" "时间轴" "图片记录可渲染" "需真机验证" "docs/NEARBY_DUAL_DEVICE_UAT.md（图片跨端同步后检查）"

render_row "S-1" "Nearby同步" "缺失事件同步与去重" "自动已验证" "test/sync_session_test.dart"
render_row "S-2" "Nearby同步" "靠近自动同步（发现+连接+自动请求）" "需真机验证" "docs/NEARBY_DUAL_DEVICE_UAT.md"
